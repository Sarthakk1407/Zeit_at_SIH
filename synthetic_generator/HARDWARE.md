...
| **Raspberry Pi 5** | Lane B compute | with official Active Cooler, 27 W PSU, 2 × microSD — the cooler and PSU are mandatory (`architecture.md` §9) |
| **Lane A module** | FxLMS loop, < 1 ms | separate DSP board inside the earcup, never the Pi. Tentative 34 × 26 mm layout: `presentation/hw/pcb-lane-a.png` (drawn by `presentation/make_pcb.py`) |
| **Communication unit** | radio sink | the chain ends at a radio (MELPe), not a screen |
...
|---|---|---|
| `P_MIC1` | (−1.6, −5.6, 11.2) | **boom mic at the mouth** — primary (Lane B) |
| `P_MIC2` | (12.1, −0.2, 0.4) | **outer earcup shell** — noise reference (Lanes A and B) |
| `P_MIC3` | (8.8, 1.0, 2.0) | **inside the earcup** — in-ear error sensor (Lane A only) |
| `P_SPK` | (9.8, −0.2, 0.6) | earcup driver — anti-noise output |
| `P_DSPA` | (11.6, −3.4, −1.8) | **Lane A module** — FxLMS board clipped to the outside of the right cup |
| `P_EAR` | (8.6, −0.6, 0.4) | the ear canal the loop protects |

The boom arm is on the left cup; the right cup carries Mic 2, Mic 3, the driver
and the Lane A module, and is the one shown cut away.
...
| +0.55 ms | Lane A converged — in-ear residual 18.4 dB below the reference (budget 1 ms) |
